...
add_action( 'after_setup_theme', 'meris_setup' );
 ...
 function meris_custom_scripts(){
	...
	wp_enqueue_style('meris-carousel',  get_template_directory_uri() .'/css/owl.carousel.css', false, '1.3.2', false);
	wp_enqueue_style('meris-bigvideo',  get_template_directory_uri() .'/css/bigvideo.css', false, '', false);
	wp_enqueue_style( 'meris-main', get_stylesheet_uri(), array(), $theme_info->get( 'Version' ) );
	wp_enqueue_style('meris-scheme', get_template_directory_uri().'/css/scheme.less', false, $theme_info->get( 'Version' ), false);
   ...
   $meris_custom_css = "";
	...
	if( $home_banner != 1 && $home_banner != 2 ){
	$meris_custom_css .=  '.homepage header {position: relative;  background-image: url("'.get_template_directory_uri().'/images/banner-1.jpg");   background-position: center top;}';	
	}
	if ( isset($header_image) && ! empty( $header_image ) && $home_banner != 1 && $home_banner != 2 ) {
	$meris_custom_css .= ".homepage header{background:url(".$header_image. ") no-repeat;}\n";
	}
	...
	wp_enqueue_script( 'meris-carousel', get_template_directory_uri().'/js/owl.carousel.min.js', array( 'jquery' ), '1.3.2 ', false );
	wp_enqueue_script( 'meris-respond', get_template_directory_uri().'/js/respond.min.js', array( 'jquery' ), '', false );
	wp_enqueue_script( 'meris-main', get_template_directory_uri().'/js/meris.js', array( 'jquery' ),$theme_info->get( 'Version' ), true );
	...
	$slide_type         = "image";
	for($i=1;$i<=5;$i++){
		$slide_background_type  = meris_options_array('meris_slide_type_'.$i);
		if( $slide_background_type == "video" ){
			$slide_type = "video";
			}
		}
	if ( $enable_home_page !="" && ($home_banner == 2 || $slide_type == "video" ) && is_home() ) {
		
		$background_mp4_url  = meris_options_array('background_mp4_url');
		$background_ogv_url  = meris_options_array('background_ogv_url');
		$background_webm_url = meris_options_array('background_webm_url');
		$background_poster   = meris_options_array('background_poster');
		$background_loop     = meris_options_array('background_loop');
		
		wp_localize_script( 'meris-main', 'video_background',  array(
			'mp4' => $background_mp4_url,
			'ogv' => $background_ogv_url,
			'webm' => $background_webm_url,
			'poster' => $background_poster,
			'loop'  => $background_loop
		)  );
		
	wp_enqueue_script( 'meris-video', get_template_directory_uri().'/js/video.js', array( 'jquery' ), '', false );
	wp_enqueue_script( 'meris-bigvideo', get_template_directory_uri().'/js/bigvideo.js', array( 'jquery' ), '', false );
	}
	//
 ...
 $location = apply_filters( 'options_framework_location', array('admin-options.php') );
	...
	//default home page sections
	 if(!get_option( '_meris_home_widget_area' )){
		...
		$default_sections_json = '{"section-widget-area-name":["Home Page Secion One","Home Page Secion Two","Home Page Secion Three","Home Page Secion Four"],"list-item-color":["","","",""],"list-item-image":["","","","'.MERIS_THEME_BASE_URL.'/images/background-1.jpg"],"list-item-repeat":["","","","no-repeat"],"list-item-position":["","","",""],"list-item-attachment":["","","",""],"widget-area-layout":["boxed","full","boxed","boxed"],"widget-area-padding":["50","50","50","50"],"widget-area-column":["1","1","1","2"],"widget-area-column-item":{"home-page-secion-one":["12"],"home-page-secion-two":["12"],"home-page-secion-three":["12"],"home-page-secion-four":["6","6"]}}';
			
		add_option('_meris_home_widget_area' ,$default_sections_json);
...
function meris_optionscheck_options_menu_params( $menu ) {
	...
	$menu['page_title'] = __( 'Meris Options', 'meris');
	$menu['menu_title'] = __( 'Meris Options', 'meris');
