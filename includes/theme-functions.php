...
<?php
 ...
 function meris_get_default_slider(){
	...
	$return = '<section class="homepage-slider"><div id="carousel-meris-generic" style="'.$slide_height.'" class="carousel slide" data-interval="'.$slide_time.'" data-ride="carousel">';
	$j = 0;
	 for($i=1;$i<=5;$i++){
	$active = '';
	// $title = meris_options_array('meris_slide_title_'.$i);
	 ...
	 if($j==0) $active     = 'active';
		 ...
		 if(trim($link) == ""){
          ...
          $slideContent .= '<img src="'.$image.'" alt="" />';
		  
		 }else{
		  $slideContent .= '<a href="'.esc_url($link).'" target="_blank"><img src="'.$image.'" alt="" /></a>';
			 } 
			 
          $slideContent .= '<div class="carousel-caption">'.$text.'</div></div>';
		  $j++;
	  }
...
function meris_options_panel_sidebar() { ?>
	<div id="optionsframework-sidebar">
		<div class="metabox-holder">
	    	<div class="postbox">
	    		<h3><?php _e( 'Quick Links', 'meris' ); ?></h3>
      			<div class="inside"> 
		          <ul>
                  ...
                  <li><a href="<?php echo esc_url( 'http://www.mageewp.com/forums/meris-theme/' ); ?>" target="_blank">Support Forums</a></li>
                  </ul>
      			</div>
	    	</div>
	  	</div>
...
<?php
 ...
 /**
	 ...
	 * author: quan
	 */
 function meris_blog_shortcode($atts,$content=NULL){
  ...
  'category' => ''
	...
	$wp_query -> query('category_name='.$category.'&paged='.$paged."&post_status=publish"); 
	$i = 1 ;					
	if ($wp_query -> have_posts()) :
    
    $return = '<div class="shortcode-blog blog-list-wrap">';
	...
	endwhile;
	 ...
	 $return .= '<div class="list-pagition text-center">'.meris_native_pagenavi("",$wp_query).'</div>';
  ...
  add_shortcode('blog','meris_blog_shortcode');